...
class EditAdv(form.EditForm):
    ...
    def getFields(self):
        ...
        for behavior in self.context.portal_types[self.context.portal_type].behaviors:  # noqa
    ...
    def updateWidgets(self):
        ...
        self.fields['image'].field.description = u'foto/immagine dell\'annuncio'  # noqa

        self.widgets['helptext'].mode = HIDDEN_MODE
        self.widgets['privacytext'].mode = HIDDEN_MODE

    @button.buttonAndHandler(_('Save', default=u'Applica'), name='save')
    def handleAdd(self, action):
        ...
        if data['ads_category'] != self.context.aq_parent.UID():
            # move object to new category
            api.content.move(source=self.context, target=container)

        self.applyChanges(data)

        self.immediate_view = u'/'.join(
            [container.absolute_url(), self.context.id]
        )
        self.request.response.redirect(self.immediate_view)
